...
public class CameraConstantSize : MonoBehaviour
{
    ...
    private void Update()
    {
        if (componentCamera.orthographic)
        {
            ...
            componentCamera.orthographicSize = Mathf.Lerp(constantWidthSize, initialSize, WidthOrHeight) + FieldOfViewOffset; ;
        }
        else
        {
            float constantWidthFov = CalcVerticalFov(horizontalFov, componentCamera.aspect);
            componentCamera.fieldOfView = Mathf.Lerp(constantWidthFov, initialFov, WidthOrHeight) + FieldOfViewOffset;
        }
    }
